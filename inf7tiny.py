...
model = YoloTRT(
    ...
    engine="yolov7/build/best-yolov7-tiny.engine",
    conf=0.5,
    yolo_ver="v7",
)
...
def network_share_auth():
    smbclient.shutil.copyfile(
        f"{cwd}/DataStorage/check.txt",  # Eg - /mnt/myfolder/check.txt
        "\\\\192.168.3.200\\share\\Jeson-nano\\CheckRun\\check.txt",  # Eg \\CDTPS\check.txt
        username="HDPL",  # Username of the the user who have access to the Network drive
        password="Hondapluskt2021",
    )  # Password of the the user who have access to the Network drive


def Send_Image_NG():
    smbclient.shutil.copyfile(
        f"{cwd}/DataStorage/tmp_image.jpg", 
        "\\\\192.168.3.200\\share\\Jeson-nano\\CheckRun\\image.jpg", 
        username="HDPL", 
...
def Send_diff_case():
    smbclient.shutil.copyfile(
        ...
        "\\\\192.168.3.200\\share\\Jeson-nano\\CheckRun\\diff_case\\image.jpg",  # Eg \\CDTPS\Test.txt
        username="HDPL",  # Username of the the user who have access to the Network drive
        password="Hondapluskt2021",
    )  # Password of the the user who have access to the Network drive
    # print(f'{cwd3}/DataStorage/tmp_image.jpg')
...
def update_back3_threshold(val):
    global dist_back3_threshold
    dist_back3_threshold = val
...
cv2.createTrackbar(
    "Back3 Threshold", "Parameters", dist_back3_threshold, 500, update_back3_threshold
)
    ...
    try:
        ...
        for obj in detections:
            ...
            if name == "bottleneck":
                ...
                count += 1
            elif name == "back_3" and check_back3:
                back3.append(obj["box"][0])
                back3.append(obj["box"][1])
                back3.append(obj["box"][2])
                back3.append(obj["box"][3])
                count += 1
            elif name == "middle" and check_middle:
                middle.append(obj["box"][0])
                middle.append(obj["box"][1])
                middle.append(obj["box"][2])
                middle.append(obj["box"][3])
        ...
        if check_has_bottleneck > 0 and count > 1:
            if len(bottleneck) > 0 and len(front) > 0:
                ...
                centroid_back2 = (
                    int(back2[0] + (back2[2] / 2)),
                    int(back2[1] + (back2[3] / 2)),
                )

                center_x1, center_y1 = get_line_center(
                    int(back2[0]), int(back2[1]), int(back2[2]), int(back2[3])
                )
                center_x2, center_y2 = get_line_center(
                    int(bottleneck[0]),
                    int(bottleneck[1]),
                ...
                dist_neck_to_back2 = calculate_distance(
                    center_x2, center_y1, center_x2, center_y2
                )

                cv2.putText(
                    frame,
                    "Back_2 : " + str(dist_neck_to_back2),
                    (0, 250),
                    cv2.FONT_HERSHEY_PLAIN,
                    3,
                    (0, 255, 255),
                ...
                centroid_back3 = (
                    int(back3[0] + (back3[2] / 2)),
                    int(back3[1] + (back3[3] / 2)),
                )

                center_x1, center_y1 = get_line_center(
                    int(back3[0]), int(back3[1]), int(back3[2]), int(back3[3])
                )
                center_x2, center_y2 = get_line_center(
                    int(bottleneck[0]),
                    int(bottleneck[1]),
                ...
                dist_neck_to_back3 = calculate_distance(
                    center_x2, center_y1, center_x2, center_y2
                )

                cv2.putText(
                    frame,
                    "Back_3 : " + str(dist_neck_to_back3),
                    (0, 250),
                    cv2.FONT_HERSHEY_PLAIN,
                    3,
                    (0, 255, 255),
        ...
        if 300 > dist_neck_to_front > dist_front_threshold:
            message = "Wrong"
        elif 0 < dist_neck_to_front < dist_front_threshold:
            message = "OK"
        elif 300 > dist_neck_to_back > dist_back_threshold:
            message = "OK"
        elif 0 < dist_neck_to_back < dist_back_threshold:
            message = "Wrong"
        elif 300 > dist_neck_to_back2 > dist_back2_threshold:
            message = "Wrong"
        elif 0 < dist_neck_to_back2 < dist_back2_threshold:
            message = "OK"
        elif 300 > dist_neck_to_back3 > dist_back_threshold:
            message = "OK"
        ...
        directory4 = (
            cwd + f"/DataStorage/{str(year)}/{str(month)}/{str(today)}/ImageError"
        )
        ...
        if (
            ...
            # Lưu ảnh vào thư mục diff_case
            diff_case_dir = f"{directory3}/diff_case"
            if not os.path.isdir(diff_case_dir):
                os.mkdir(diff_case_dir)
                open(f"{diff_case_dir}/Distance.txt", "x")
                open(f"{diff_case_dir}/log.txt", "x")
            diffcase = cv2.imread(f"{cwd}/DataStorage/tmp_image.jpg")
            cv2.imwrite(f"{diff_case_dir}/diff_case-{current_time}.jpg", diffcase)
            Send_diff_case()

        # Check if any distance is greater than 300
        # Check if any distance is greater than 300
        if (
            dist_neck_to_front >= 300
            or dist_neck_to_back >= 300
            or dist_neck_to_back2 >= 300
            or dist_neck_to_back3 >= 300
        ):
            # Lưu thông tin và hình ảnh vào thư mục long_case
            long_case_dir = f"{directory3}/long_case"
            if not os.path.isdir(long_case_dir):
                os.mkdir(long_case_dir)

            # Lưu hình ảnh
            cv2.imwrite(f"{long_case_dir}/long_case-{current_time}.jpg", image)

            with open(f"{long_case_dir}/long_case-{current_time}.txt", "a") as f:
                if dist_neck_to_front > 300:
                    f.write(
                        f"Long distance detected in front: {dist_neck_to_front} at {current_time}\n"
                    )
                if dist_neck_to_back > 300:
                    f.write(
                        f"Long distance detected in back: {dist_neck_to_back} at {current_time}\n"
                    )
                if dist_neck_to_back2 > 300:
                    f.write(
                        f"Long distance detected in back2: {dist_neck_to_back2} at {current_time}\n"
                    )
                if dist_neck_to_back3 > 300:
                    f.write(
                        f"Long distance detected in back3: {dist_neck_to_back3} at {current_time}\n"
                    )

            # Gửi hình ảnh qua mạng
            Send_long_case()
                ...
                if check_2:
                    ...
                    if results_wrong > results_ok and results_wrong >= 3:
                        ...
                        if dist_neck_to_front > 0:
                            ...
                            f.write(
                                f"NG in front at {current_time} -- distance: {dist_neck_to_front} \n"
                            )
                            f.close()
                            # cv2.imwrite(f"{directory4}/NG-{current_time}.jpg", errorImage)
                            ...
                            f = open(f"{directory3}/Distance.txt", "a")
                            f.write(
                                f"NG in back at {current_time} -- distance: {dist_neck_to_back} \n"
                            )
                            f.close()
                            # cv2.imwrite(f"{directory4}/NG-{current_time}.jpg", errorImage)
                            ...
                            f = open(f"{directory3}/Distance.txt", "a")
                            f.write(
                                f"NG in back2 at {current_time} -- distance: {dist_neck_to_back2} \n"
                            )
                            f.close()
                            # cv2.imwrite(f"{directory4}/NG-{current_time}.jpg", errorImage)
                        elif dist_neck_to_back3 > 0:
                            f = open(f"{directory3}/Distance.txt", "a")
                            f.write(
                                f"NG in back3 at {current_time} -- distance: {dist_neck_to_back3} \n"
                            )
                            f.close()
                        check_2 = False
